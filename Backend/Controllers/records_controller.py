...
class MedicalRecordAPI(Resource):
    def get(self, patient_id=None):
        if patient_id:
            ...
            if medical_records:
                ...
                app.logger.info(f"Medical records: {response_data}")
        ...
        app.logger.info(f"No medical records found for the specified patient: {patient_id}")
    ...
    def post(self):
        ...
        try:
            ...
            app.logger.info(f"Medical record created successfully: {new_medical_record.id}")
            ...
            app.logger.info(f"Medical record not created: {new_medical_record.id}")
    ...
    def put(self, medical_record_id):
        ...
        if medical_record:
            try:
                ...
                app.logger.info(f"Medical record updated successfully: {medical_record.id}")
                return Response(json.dumps(response_data), mimetype="application/json", status=200)
            except Exception as e:
                db.session.rollback()
                response_data = {
                ...
                app.logger.info(f"Medical record not updated: {medical_record.id}")
                return Response(json.dumps(response_data), mimetype="application/json", status=400)
        response_data = {
            "data": {
        ...
        app.logger.info(f"Medical record not found: {medical_record.id}")
        return Response(json.dumps(response_data), mimetype="application/json", status=404)
    ...
    def delete(self, medical_record_id):
        ...
        if medical_record:
            ...
            app.logger.info(f"Medical record deleted successfully: {medical_record.id}")
            return Response(json.dumps(response_data), mimetype="application/json", status=200)
        response_data = {
            "data": {
                "message": "Medical record not found",
